...
const sendSMS = async (mobile, name, otp) => {
  ...
  try {
    // VARIATION 1: Standard Key-Value (Did not work for delivery despite success response?)
    ...
    // VARIATION 2: OTP as Array (Suggested by user comments?)
    const payloadArray = {
        ...payloadStandard,
        otp: [otp] // Trying array
    };

    console.log('Payload (Array Variation):', JSON.stringify(payloadArray, null, 2));
