...
const hasOpenOrUpcomingAssessmentWindow = (application = {}) => {
  ...
  if (
    ['completed', 'expired', 'suspended', 'passed', 'failed'].includes(assessmentStatus) ||
    ['pass', 'fail', 'passed', 'failed'].includes(assessmentResult)
  ) {
    return false;
  }
...
const shouldAutoRejectExpiredApplication = (application = {}) => {
  ...
  if (hasOpenOrUpcomingAssessmentWindow(application)) {
    return false;
  }
  ...
  try {
    ...
    if (normalizedInterviewProcess?.stages?.length) {
      ...
      const orderedAssessmentIds = orderedAssessmentRoundKeys.reduce((acc, roundKey) => {
        ...
        const roundAssessmentId = roundDetails?.assessmentId || null;
