  ...
  try {
    ...
    for (let index = 0; index < jsonData.length; index++) {
      try {
        ...
        if (normalizedEmail && emailsProcessedInFile.has(normalizedEmail)) {
          skippedCount++;
          continue;
        }
        if (normalizedEmail) {
        ...
        // Check if user already exists in any role
        const existingUser = await checkEmailExists(normalizedEmail || email);
        if (existingUser) {
          skippedCount++;
          continue;
        }
  ...
  try {
    ...
    res.json({ 
      ...
      placement: {
        ...
        phone: placement.phone,
        collegeName: placement.collegeName,
        collegeAddress: placement.collegeAddress,
        collegeOfficialEmail: placement.collegeOfficialEmail,
        additionalOfficialEmail: placement.additionalOfficialEmail,
        collegeOfficialPhone: placement.collegeOfficialPhone,
...
exports.sendOTP = async (req, res) => {
  try {
    ...
    console.error('Placement sendOTP error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};
