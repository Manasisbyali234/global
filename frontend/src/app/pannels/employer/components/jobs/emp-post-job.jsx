...
export default function EmpPostJob({ onNext }) {
	...
	const getMinDateForRoundFromState = (state, roundKey) => {
		const baseMin = getAssessmentMinDate(state.lastDateOfApplication);
		if (!roundKey) return baseMin;

		const order = state.interviewRoundOrder || [];
		const roundIndex = order.indexOf(roundKey);
		if (roundIndex <= 0) return baseMin;

		// Find the latest date among all previous rounds
		let prevMax = '';
		for (let i = 0; i < roundIndex; i++) {
			const prevKey = order[i];
			const prevDate = normalizeToYMD(state.interviewRoundDetails?.[prevKey]?.fromDate || '');
			if (prevDate && prevDate > prevMax) prevMax = prevDate;
		}

		if (!prevMax) return baseMin;
		const nextDay = getNextDayDateString(prevMax);
		return nextDay > baseMin ? nextDay : baseMin;
	};
	...
	const updateRoundDetails = async (roundType, field, value, skipHolidayConfirmation = false) => {
		...
		if (field === 'fromDate' && value) {
			...
			if (minAllowedInterviewDate && value < minAllowedInterviewDate) {
				if (applicationMinDate === minAllowedInterviewDate) {
					showError(`${isAssessment ? 'Assessment' : 'Interview'} date must be after the Last Date of Application. Please select ${formatDate(minAllowedInterviewDate)} or later.`);
				} else {
					showError(`This round must be scheduled on ${formatDate(minAllowedInterviewDate)} or later (next day after the previous interview round).`);
				}
				return;
			}
		...
		setFormData(s => {
			...
			if (field === 'fromDate' && value) {
				...
				if (minAllowedInterviewDate && value < minAllowedInterviewDate) {
					if (applicationMinDate === minAllowedInterviewDate) {
						showWarning(`${isAssessmentRound ? 'Assessment' : 'Interview'} date must be after the Last Date of Application. Please select ${formatDate(minAllowedInterviewDate)} or later.`);
					} else {
						showWarning(`This round must be scheduled on ${formatDate(minAllowedInterviewDate)} or later (next day after the previous interview round).`);
					}
					return s;
				}
		...
		for (const uniqueKey of selectedRounds) {
			...
			if (!details?.fromDate) {
				...
				if (minAllowedInterviewDate && dateStr < minAllowedInterviewDate) {
					if (applicationMinDate === minAllowedInterviewDate) {
						errorMessages.push(`${roundName} must be scheduled after the Last Date of Application. Select ${formatDate(minAllowedInterviewDate)} or later.`);
					} else {
					...
					if (!subStage.fromDate) {
						...
						if (subStage.fromDate < minAllowedInterviewDate) {
							...
							if (index === 0 && minAllowedInterviewDate === applicationMinDate) {
								errorMessages.push(`${roundName} - Day ${index + 1} must be scheduled after the Last Date of Application. Select ${formatDate(minAllowedInterviewDate)} or later.`);
							} else {
																			...
																			if (startTime && endTime) {
																				...
																				if (slots && slots > 0) {
																					...
																					if (totalMinutes < requiredMinutes) {
																						showError(`Total interview time (${(totalMinutes/60).toFixed(1)} hours) must be â‰¥ ${(requiredMinutes/60).toFixed(1)} hours (${slots} slots Ã— 1 hour per candidate)`);
																						return;
																					}
																				}
																			}
