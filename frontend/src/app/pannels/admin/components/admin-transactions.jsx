                ...
                <div className="panel panel-default site-bg-white p-4" style={{ background: 'white', borderRadius: '12px', border: '1px solid #eef2f7', boxShadow: 'none', margin: 0 }}>
                            ...
                            <table className="table table-hover twm-table">
                                ...
                                <tbody>
                                    {filteredTransactions.length === 0 ? (
                                        ...
                                        filteredTransactions.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE).map((t) => (
                                            <tr key={t._id}>
                                                <td>
                    ...
                    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", marginTop: "16px", borderTop: "1px solid #e9ecef", paddingTop: "14px", flexWrap: "wrap", gap: "10px", flexDirection: "column" }}>
                        <div style={{ color: "#6c757d", fontSize: "13px" }}>
                            Showing {filteredTransactions.length === 0 ? 0 : (currentPage - 1) * PAGE_SIZE + 1}–{Math.min(currentPage * PAGE_SIZE, filteredTransactions.length)} of {filteredTransactions.length} transaction{filteredTransactions.length !== 1 ? "s" : ""}
                        </div>
                        {Math.ceil(filteredTransactions.length / PAGE_SIZE) > 1 && (
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", flexWrap: "wrap" }}>
                                ...
                                {Array.from({ length: Math.ceil(filteredTransactions.length / PAGE_SIZE) }, (_, i) => i + 1).map(page => (
                                ...
                                <button onClick={() => setCurrentPage(p => p + 1)} disabled={currentPage === Math.ceil(filteredTransactions.length / PAGE_SIZE)} style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: "34px", height: "34px", borderRadius: "6px", border: "1px solid #dee2e6", background: currentPage === Math.ceil(filteredTransactions.length / PAGE_SIZE) ? "#f8f9fa" : "#fff", color: currentPage === Math.ceil(filteredTransactions.length / PAGE_SIZE) ? "#adb5bd" : "#495057", cursor: currentPage === Math.ceil(filteredTransactions.length / PAGE_SIZE) ? "not-allowed" : "pointer", fontSize: "13px", fontWeight: 600 }}>&#8250;</button>
                            </div>
                        )}
                    </div>
                </div>
                                            ...
                                            <div className="col-8">
                                                <div className="p-3 rounded border bg-light" style={{ borderLeft: '4px solid #f97316 !important' }}>
                                                    ...
                                                    <ul className="list-unstyled small text-muted mb-0" style={{ fontSize: '11px' }}>
                                                        ...
                                                        <li>• For support, please contact support@taleglobal.net with your Transaction ID.</li>
                                                    </ul>
                                                </div>
                                            </div>
